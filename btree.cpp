...
using namespace std;
int a;
void delimtextbuffer::pack(car& p)
{
    strcpy(buffer, p.id);
	strcat(buffer, "|");
	strcat(buffer, p.name);
	strcat(buffer, "|");
    strcat(buffer, p.DLno);
	strcat(buffer, "|");
    strcat(buffer, p.cartype);
	strcat(buffer, "|");
    strcat(buffer, p.Bdate);
	strcat(buffer, "|");
	strcat(buffer, "*");
}
void delimtextbuffer::unpack(car& p)
{
	char* ptr = buffer;
...
searchqt operations::search(int offset)
{
	...
	strcpy(s.n, p.name);
    strcpy(s.d, p.DLno);
    strcpy(s.c, p.cartype);
    strcpy(s.b, p.Bdate);
	file1.close();
	return s;
}

getvalues operations::insert(car & ob)
{
	getvalues g;
	delimtextbuffer buff;
	int offset;
...
QList<searchqt> btree::traversed(node * ptr)
{
	searchqt disp;
    QList<searchqt> display1;
    QList<searchqt> display2;
    searchqt x;
    strcpy(x.i," ");
    strcpy(x.n," ");
    strcpy(x.d," ");
	...
	for (int i = 0; i < ptr->size; i++) {
		...
		{
			disp = o.search(ptr->addr[i]);
            cout<<disp.i<<endl;
            display1.append(disp);
            cout<<display1[i].d<<endl;
